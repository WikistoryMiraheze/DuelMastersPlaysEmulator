...
def spell_effect(
    ...
    if power > 4000:
        ...
        raise GameError("対象のパワーは4000以下でなければなりません。")
...
def prompt_targets(game: "Game", caster: "Player") -> Optional[Sequence["Card"]]:
    return game.prompt_power_limited_target(caster, 4000)
...
def auto_targets(game: "Game", caster: "Player") -> Optional[Sequence["Card"]]:
    ...
    for creature in opponent.battle_zone:
        if creature.is_creature() and game._calculate_card_power(opponent, creature) <= 4000:
            return (creature,)
    return ()
